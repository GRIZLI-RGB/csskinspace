...
const NavigationList = ({
	...
	return (
		<nav className={clsx("flex flex-col gap-3 items-start max-sm:text-center max-sm:items-center", className)}>
			{title && (
...
export default function Footer() {
	return (
		<footer className="border-t border-primaryBorder">
			...
			<div className="max-sm:text-center max-sm:gap-6 max-sm:flex-col border-t border-primaryBorder text-secondaryText px-8 py-6 flex items-center justify-between text-xs gap-12">
				<p>
					YOURGAMES LTD Company number 16382696 Registered office
					address: 86-90 Paul Street London, Greater London, England,
					EC2A 4NE
				</p>
				<p className="text-right max-sm:text-center">
					CsSkinSpace website is operated by In-Game Solutions PTE.
					LTD © 2020-2025 CsSkinSpace, All Rights Reserved
				</p>
			</div>
		</footer>
	);
